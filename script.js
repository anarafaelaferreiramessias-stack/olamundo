        ...
        function init() {
            ...
            // Nuvens
            for(let i=0; i<15; i++) {
                const cloud = new THREE.Mesh(new THREE.BoxGeometry(15, 1, 10), new THREE.MeshLambertMaterial({color: 0xffffff, transparent: true, opacity: 0.7}));
                cloud.position.set(Math.random()*200-100, 40, Math.random()*200-100);
                scene.add(cloud);
                clouds.push(cloud);
            }
        ...
        function placeBlock() {
            ...
            if (hits.length > 0 && hits[0].distance < 5) {
                const hit = hits[0];
                // Calcula a posição vizinha usando a Normal da face atingida
        ...
        function animate() {
            ...
            // Nuvens
            clouds.forEach(c => { c.position.x += 0.03; if(c.position.x > 150) c.position.x = -150; });
